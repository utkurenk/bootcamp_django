...
def income(request):
    ...
    return render(request, 'gelir.html', context)


@login_required(login_url="/login")
def add_one(request):
    x = request.POST['Amount']
    transactions = Transaction(Type="Gelir", Amount=x)
...
def expense(request):
    ...
    return render(request, 'gider.html', context)
...
def add_two(request):
    ...
    transaction = Transaction(Type="Gider", Amount=x, Category=y)
